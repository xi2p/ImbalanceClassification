...
from loss import FocalLoss, compute_class_weights   # 新增
...
def parse_args():
    ...
    parser.add_argument('--gamma', type=float, default=2.0,
                        help='Focal Loss 的聚焦参数')
    parser.add_argument('--alpha_mode', type=str, default='inv_sqrt',
...
def main():
    ...
    class_weights = compute_class_weights(
        train_set.labels, args.num_classes, mode=args.alpha_mode
    )
    if class_weights is not None:
        class_weights = class_weights.to(device)
        logger.info(f'Class weights (mode={args.alpha_mode}) '
                    f'min={class_weights.min():.4f}, '
                    f'max={class_weights.max():.4f}')
    criterion = FocalLoss(
        alpha=class_weights, gamma=args.gamma, reduction='mean'
    ).to(device)
    logger.info(f'Using FocalLoss(gamma={args.gamma}, '
