...
from loss import CBLoss, compute_cb_weights
...
def main():
    ...
    cb_weights = compute_cb_weights(
        train_set.labels, args.num_classes, beta=args.beta, normalize=True
    )
    logger.info(f'CB Loss | beta={args.beta}, gamma={args.gamma}')
    logger.info(f'Class counts min={counts.min()}, max={counts.max()}, '
                f'imbalance ratio={counts.max() / max(counts.min(), 1):.1f}')
    logger.info(f'CB weights  min={cb_weights.min():.4f}, '
                f'max={cb_weights.max():.4f}, '
                f'mean={cb_weights.mean():.4f}')
